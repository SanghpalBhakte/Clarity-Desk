...
- **New suite**: `tests/verify_ocr_scan_improvements.mjs` (15 scenarios) covers the ported pieces (dark-invert threshold, legend parsing including not misreading the adjacent faculty-legend table, digit-confusion correction, and `subjectLegend` threading end-to-end through `reconstructTimetable2DGrid`). Wired into `npm test` and `release:check`. The deskew/local-contrast/sharpen/vision-fallback pieces built directly in this session don't have image-level tests — see the known limitation below.
