...
- **New suite**: `tests/verify_ocr_scan_improvements.mjs` (28 scenarios as of the root-cause pass below) — unit tests for all of the above as pure/near-pure functions with synthetic fixtures (no image files, no network, no paid APIs — consistent with every other suite in this project). Wired into `npm test` and `release:check`.
